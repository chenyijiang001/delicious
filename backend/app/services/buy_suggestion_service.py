...
ALIAS_PROMPT = """你是食材归类助手。对输入的食材名列表，对每一项返回严格 JSON：

输入示例：["西红柿", "紫苏", "鸡蛋"]
输出（严格 JSON 数组，与输入顺序一一对应）：
[
  {"alias":"西红柿","canonical":"番茄","category":"蔬菜",
   "store_type_coverage":{"supermarket":true,"convenience":false,"market":true,"fresh":true},
   "confidence":0.95},
  {"alias":"紫苏","canonical":"紫苏","category":"蔬菜",
   "store_type_coverage":{"supermarket":false,"convenience":false,"market":true,"fresh":true},
   "confidence":0.9},
  {"alias":"鸡蛋","canonical":"鸡蛋","category":"肉蛋",
...
class BuySuggestionService:
    ...
    async def build(
        ...
        # 2. 食材语义归类
        aliases = await self._resolve_aliases(db, items)
        ...
        return {
            ...
            "cache_hit": {
                "alias": aliases.get("__cache_hit__", False),
                "poi": False if poi_stale else (not offline_blocks or True),
                "coverage": True,  # 覆盖度纯规则，标记命中即可
                "suggestion": ai_suggestion is not None
                and "__from_cache__" in (ai_suggestion or ""),
            },
        }
