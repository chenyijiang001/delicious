...
- 若图片中并未出现食物，请返回 {"unrecognized": true}"""


class NoFoodDetected(Exception):
    """AI 判定图片中没有食物。对应 HTTP 422 + code=no_food_detected。"""
...
class AIService:
    ...
    async def recognize_food(self, image_bytes: bytes) -> tuple[dict, bool, int]:
        """返回 (result, cache_hit, latency_ms)。"""
        ...
        if self.redis:
            try:
                ...
                if cached:
                    return json.loads(cached), True, int((time.perf_counter() - t0) * 1000)
            except Exception:
                pass  # Redis 故障不影响主流程
        ...
        return result, False, int((time.perf_counter() - t0) * 1000)
